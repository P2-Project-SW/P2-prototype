...
function testForK(data, kValues) {
    const WCSS = []; // Dette er din Inertia
    const distortions = []; // Dette er din Distortion
    kValues.forEach(k => {
...
function findElbowManually(K, inertias) {
    ...
    for (let i = 0; i < K.length; i++) {
        ...
        const numerator = Math.abs((p2.y - p1.y) * p0.x -
            ...
            p2.x * p1.y -
            p2.y * p1.x);
        const denominator = Math.sqrt(Math.pow(p2.y - p1.y, 2) +
...
if (elbowGraph) {
    if (data1 && data1.length > 0) {
        Plotly.newPlot(elbowGraph, data1, layout2D);
    }
}
else {
...
if (dataGraph) {
    if (data2 && data2.length > 0) {
        Plotly.newPlot(dataGraph, data2, layout3D);
    }
}
else {
